...
): Promise<ResourceHandle<MaterialResource> | null> {
  try {
    if (!shaderHandle) {
    ...
    // Use the factory system to create the material
    const handle = resourceManager.createResource<MaterialResource>('material', name, materialData);

    if (!handle) {
      throw new Error(`Failed to create material resource using factory`);
    }
...
): ResourceHandle<MeshResource> | null {
  try {
    // Use the existing factory system instead of direct instantiation
    const handle = resourceManager.createResource<MeshResource>('mesh', name, descriptor);

    if (!handle) {
      throw new Error(`Failed to create mesh resource using factory`);
    }
